...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    zh: '给两个老模型补上了 superseded 标记（这是 QTZ-101 里体量小、风险低的那一半——它另一个提议，做一个跨模型的 qualityScore、靠外部排行榜打分，需要站主自己选一个可信的数据源，不在这次范围内）：Mixtral 8x7B Instruct → Qwen3 30B-A3B（上下文 40K 对 32K，Q4 显存 19GB 对 28.5GB，更快，实测质量损失更低）；Stable LM 2 12B Chat → Falcon 3 10B Instruct（上下文 32K 对已经过时的 4K，文件更小，更快）。两条理由都是 supersededDiffNote() 从真实字段算出来的，不是套模板，两个页面也都还是 index, follow、数字照旧——只是 homePicks / /best/ 不会再第一个推荐它们。同一批审计里还有两个候选，核实过之后特意没打标——不是图省事跳过：DeepSeek-V2-Lite Chat 现在的上下文（163,840 token）和 Q4 显存占用（11GB）在同量级里没有别的模型能比过，它在索引里唯一的同系亲属是一个专攻代码的变体，不是一次代际升级——如果强行标它被一个上下文更短的模型"取代"，就正好重演了早前那一批里 Command R 35B 选错继任者的错误。Falcon 3 10B 本来就是唯一合适的替代对象，这也是它被选为 Stable LM 继任者的原因。',
  },
  {
    date: '2026-09-21',
    en: 'Fixed the two foundational bugs flagged in the batch-1 pass earlier today (QTZ-100, QTZ-106), both in the single function nearly every recommendation on this site runs through — `fitsOnGpu()` — plus the interactive VRAM calculator, which shared neither the bug nor, until now, the fix. First: recommendations picked a quant purely by quality loss, with no idea whether the GPU could load that *format* at all — a Mac, CPU, or AMD page could recommend AWQ, GPTQ or EXL2, none of which those backends can run (AMD is a partial exception: vLLM ships official ROCm wheels, so this index still allows AWQ there, matching what `/faq/` already said before this fix existed — only EXL2 and GPTQ are excluded on AMD). Second: a Mac\'s unified memory was sized at its full nameplate figure with no reduction for what macOS actually lets the GPU wire down — real-world measurements cluster around 75% of the total, fairly consistently across machine sizes, so that\'s the figure used now (not the size-tiered 67%/75% split first proposed, which had no independent support). Both fixes are threaded through everywhere a model gets matched to hardware: `fitsOnGpu`, `countModelsFitting`, `cardsFitting`, `modelPlacement`, `bestPage` (including the Apple ladder), the GPU-page FAQ generator, `/faq/`\'s own 70B question, format pages\' "smallest card" line, and — the biggest gap — the VRAM calculator\'s reverse mode and "your card" panel, which used to run entirely separate, unfixed logic (`getRecommendations` took a bare number, not a GPU, so it had no way to know a target lacked CUDA or was unified memory). The calculator now shows an explicit "doesn\'t run on this backend" message instead of a size-based verdict when the format itself is the blocker. Measured effect: a Mac M3 16G now shows 44 of 81 models fitting comfortably, down from parity with a real 16GB CUDA card (52) — the gap is exactly the models that needed a format or capacity this card doesn\'t actually have.',
    zh: '修完了今天早些时候第一批里点出来的两个基础性 bug（QTZ-100、QTZ-106），都在几乎每一处推荐逻辑都会经过的同一个函数里——fitsOnGpu()——顺带也修了交互式显存计算器，它之前既没有这个 bug 的影子，也一直没跟着挨过修。第一个：推荐算法选量化档位只看质量损失，完全不管这张显卡的后端到底能不能加载这个格式——Mac、纯 CPU、AMD 的页面都可能推荐 AWQ、GPTQ 或 EXL2，而这几个后端里没有一个真的能跑这些格式（AMD 是个部分例外：vLLM 有官方 ROCm 构建，所以本索引在 AMD 上仍然允许 AWQ，这和这次修复之前 /faq/ 页面自己已经写明的说法是一致的——只有 EXL2 和 GPTQ 在 AMD 上被排除）。第二个：Mac 的统一内存此前是按标称满配容量计算的，完全没有扣除 macOS 实际会让 GPU 锁定多少的那部分——真实测量数据显示，无论机器大小，这个比例都比较一致地落在总容量的 75% 左右，所以现在用的就是这个数字（不是最初提议的、按容量分档的 67%/75% 那套，那套数字找不到独立来源支持）。这两处修复贯穿了所有把模型和硬件做匹配的地方：fitsOnGpu、countModelsFitting、cardsFitting、modelPlacement、bestPage（包括 Apple 阶梯）、GPU 页的 FAQ 生成器、/faq/ 自己的 70B 问题、格式页的"最小显卡"那一行——以及最大的一处缺口：显存计算器的反向模式和"你的显卡"面板，这两处此前完全跑的是另一套、从没修过的逻辑（getRecommendations 接收的是一个裸数字而不是显卡对象，根本没法知道目标显卡没有 CUDA 或者是统一内存）。现在计算器在格式本身就是障碍的时候，会明确显示"这个后端跑不了"，而不是给一个基于体积大小算出来的裁决。实际效果：Mac M3 16G 现在显示 81 个模型中有 44 个能从容运行，而不是和一张真正的 16GB CUDA 显卡（52 个）打平——差的这一截，正好就是这张卡实际上不具备的格式或容量所对应的那些模型。',
  },
  {
    date: '2026-09-21',
    en: 'A second live re-check (real browser, real fetch reads of the served HTML — a new audit pass, not a rerun of the last one) turned up a batch of real, low-risk fixes, verified against the actual codebase rather than taken on the audit\'s word: a made-up GPU name, "RTX 4070 Ti 16G" (the real 16GB card is the Ti Super), had been hand-typed into two homepage picks instead of read from the GPU database; `/about/` still advertised "79+ models" as literal text while the rest of the site had long since moved to 81, computed; the Footer\'s llama.cpp and ExLlamaV2 links still pointed at each project\'s old GitHub org (ggerganov, turboderp) rather than where they actually live now (ggml-org, turboderp-org). Also addressed a plausible cause of a hydration error the audit\'s browser console had caught but this environment cannot reproduce directly: the site\'s contact address was rendered as bare visible text in the Footer and in two feedback panels, which is exactly the pattern Cloudflare\'s Email Address Obfuscation rewrites at the edge — if the served HTML differs from what the page\'s own React bundle expects, that is a textbook hydration mismatch. Wrapped the address in Cloudflare\'s own documented `<!--email_off-->` escape comments everywhere it renders as text. This is a best-effort code-side mitigation, not a confirmed fix — it hasn\'t been checked against the live, Cloudflare-fronted site, and disabling Email Address Obfuscation in the Cloudflare dashboard remains the more certain fix, which only the site owner can do.',
    zh: '第二次线上实测复核（真实浏览器、真实抓取已发布的 HTML——是一次新的审计，不是重跑上一次）又挖出一批真实、低风险的修复，全部对照代码库核实过，不是照单全收审计报告的说法：首页两处编辑精选手写了一个不存在的显卡名"RTX 4070 Ti 16G"（真正的 16GB 版本叫 Ti Super），没有从显卡数据库里取；/about/ 页正文还写着字面文本"79+ models"，而站内其他地方早就换成了动态计算的 81；页脚的 llama.cpp 和 ExLlamaV2 链接还指向这两个项目各自的旧 GitHub 组织名（ggerganov、turboderp），而不是它们现在真正所在的地方（ggml-org、turboderp-org）。另外处理了审计报告在浏览器控制台里抓到、但这个环境没法直接复现的一个 hydration 报错的一个合理成因：本站的联系邮箱在页脚和两处反馈面板里都是以纯文本形式渲染的可见地址，这正好是 Cloudflare 的 Email Address Obfuscation 会在边缘节点改写的那种模式——如果发出去的 HTML 和页面自己的 React 代码预期的不一致，就是典型的 hydration 不匹配。已经在邮箱地址渲染成文本的每一处，套上了 Cloudflare 官方文档给出的 `<!--email_off-->` 转义注释。这是一个尽力而为的代码侧缓解措施，不是已确认生效的修复——没有条件对着真实的、走 Cloudflare 的线上站点验证过，在 Cloudflare 后台关闭 Email Address Obfuscation 仍然是更确定的解法，这一步只有站长能做。',
  },
  {
    date: '2026-09-15',
  ...
  },
  {
    ...
    zh: '把审计报告自己给出的那批高意图查询词（"4060Ti 16G 能跑什么"「Q4 掉点严重吗」「32G 内存纯 CPU 现实吗」）拿来重新核对了一遍中文站，而不是想当然认为更早的 /zh 改造已经覆盖 —— 结果基本已经覆盖：metadata 模板、FAQ 题目、硬件措辞在更早的几轮改动里就已经是按中文搜索意图重新拟的，不是机翻。找到并补上了一个真正的缺口：索引里明明已经有一行「32 GB RAM (CPU)」和一篇对应的 CPU 调优指南，却没有一条 FAQ 回答「32G 内存纯 CPU 跑大模型现实吗」——现在补上了，数字直接算自那一行数据（countModelsFitting），并老实说清楚：容量是知道的，速度不是，因为系统内存的带宽取决于读者自己的内存条配置，本站不会像给显卡标带宽那样编一个数字。另外新增了两张中文本地大模型圈子真实在淘的二手卡、此前索引里一张都没有的：Tesla P40 24G（346 GB/s，GDDR5）和 Tesla P100 16G（732 GB/s，HBM2）—— 都是 NVIDIA 官方公布、发布后从未变过的规格，归到已有的 nvidia-pro 分组、挨着 A100/H100，而不是单独建中文专属页面，因为英语圈的家庭实验室玩家同样在用这两张卡，而且这个代码库目前也没有能安全一次做完的「仅中文站」页面机制。刻意没做的一项：审计建议的「改装版 RTX 2080 Ti 22G」——它不是官方产品，22GB 来自第三方换芯片、质量参差，带宽也是推算而非规格表数字；哪怕芯片本身是真的，本站「显存靠猜的卡不能上」这条规矩在这里同样适用',
  },
  {
    date: '2026-09-14',
    en: 'GPU, model, guide, /best/ and format pages now get a real per-page share-card image instead of the one generic /og.png every page on the site used to show — 5 page types × 2 languages, generated at build time via Next\'s `opengraph-image.tsx` file convention (no runtime cost: it renders once during `next build`, same as any other static page). Building it surfaced two bugs bigger than the feature itself, both site-wide, both invisible unless you actually open a page\'s rendered HTML rather than reading the component that sets its metadata. First: `export const runtime = \'edge\'` on an `opengraph-image.tsx` silently makes Next treat the route as dynamic, and a dynamic route is silently dropped from a static export — build exits 0, no file is ever written, nothing says so. Second, and the bigger one: every page\'s `og:title`/`og:description` already varied correctly by page, but `twitter:title`/`twitter:description` did not — a hardcoded default in the root layout (and a second copy in `pageMetadata()`, inherited by the quant-hub and cookbook layouts) always won over the page\'s own text, so the Twitter/X card for literally every page on the site, including the homepage, showed the generic site tagline instead of that page\'s actual title. Fixed by deleting the hardcoded `twitter.title`/`description`/`images` everywhere and letting Twitter\'s documented fallback — read the same metadata level\'s `openGraph` fields when `twitter`\'s own are unset — do the work instead. Also added `public/_headers`: the generated image file is named `opengraph-image` with no extension, so a static host has nothing to guess its MIME type from and would serve it as `application/octet-stream` without an explicit `Content-Type: image/png` rule, which social-card validators do not sniff past',
    zh: 'GPU、模型、教程、/best/ 和格式对比页现在各自有了真正按页面生成的分享卡片图片，不再是全站共用的那张通用 /og.png —— 5 种页面类型 × 2 种语言，通过 Next 的 opengraph-image.tsx 文件约定在构建期生成（没有运行期开销：跟其他静态页面一样，只在 next build 时渲染一次）。做这个功能的过程中，顺带揪出了两个比这个功能本身影响更大的全站级 bug，而且都是那种只看设置 metadata 的组件代码根本看不出来、必须真去看页面渲染出的 HTML 才能发现的问题。第一个：opengraph-image.tsx 里写 export const runtime = \'edge\' 会让 Next 把这条路由悄悄当成动态路由处理，而动态路由在静态导出里会被直接丢弃——构建照样以 0 退出，文件却根本没生成，没有任何提示。第二个、影响更大的一个：每个页面的 og:title / og:description 早就是各自不同的了，但 twitter:title / twitter:description 却不是——根布局里写死的默认值（以及 pageMetadata() 里的另一份拷贝，被 quant-hub 和 cookbook 的布局继承）始终会盖过页面自己的文案，导致全站包括首页在内每一个页面的 Twitter/X 卡片，显示的都是那句通用的网站标语，而不是这个页面真正的标题。修复方式是把各处写死的 twitter.title / description / images 全部删掉，改为依赖 Twitter 官方文档说明的兜底规则——twitter 字段没设置时，会读取同一层级 metadata 里对应的 openGraph 字段。另外新增了 public/_headers：生成的图片文件名是不带扩展名的 opengraph-image，静态托管平台没有任何线索去猜它的 MIME 类型，如果不显式声明 Content-Type: image/png，就会被当作 application/octet-stream 提供，而社交卡片校验工具通常不会做内容嗅探',
  },
  {
    date: '2026-09-14',
    en: 'Investigated the audit\'s report of broken scroll on the mobile homepage (scrollTop stuck near 800 after a programmatic scrollTo) before touching any code, per its own instruction to reproduce first. None of its three suspected causes exist in this codebase: there is no scroll-lock logic anywhere near the mobile menu to leave uncleaned, the `#changelog` anchor already carries `scroll-mt-28` so the fixed nav cannot cover it, and there is no custom `scrollRestoration` handling to conflict with anything. A fresh headless run with a real mobile viewport, touch emulation and an actual wheel-scroll gesture (not just scrollTo) reached the footer with no issue, and a plain `scrollTo(0, maxHeight)` in that same clean environment landed exactly at the bottom — the opposite of what the audit observed. This matches the audit\'s own leading theory: a preview panel resetting scroll position, not a site defect. What could not be completed here: real-device confirmation on iOS Safari and Android Chrome, which this environment has no hardware for — that one remaining check is on the site owner, and the result above is what it needs to confirm',
    zh: '在动手改代码之前，先按审计报告自己的要求复现了一遍它提到的移动端首页滚动异常（程序化 scrollTo 之后 scrollTop 卡在 800 附近不动）。报告怀疑的三个成因，在这个代码库里一个都不存在：移动端菜单附近根本没有任何滚动锁定逻辑，谈不上"没清理干净"；#changelog 锚点本来就带着 scroll-mt-28，固定导航不可能挡住它；也没有任何自定义的 scrollRestoration 处理会跟别的逻辑冲突。用一个干净的无头浏览器环境、真实的移动端视口、触摸模拟外加真正的滚轮滚动手势（不只是 scrollTo）重新测了一遍，顺利滚到了页脚；同一个干净环境里一次普通的 scrollTo(0, 最大高度) 也精确停在了底部——跟审计报告观测到的现象正好相反。这印证了报告自己给出的主要猜测：是预览面板本身重置了滚动位置，不是网站本身的缺陷。这里做不到的一件事：在 iOS Safari 和 Android Chrome 真机上做最终确认，这个环境没有对应的硬件——剩下这最后一步要靠站主自己来做，上面的结果就是需要被确认的那个结论',
  },
  {
    date: '2026-09-14',
  ...
  },
  {
    ...
    zh: '404 页正文只有二十来个词、四个写死的链接，而这是一个有 393 个页面、URL 规律高度可猜的站点——真实访问记录里，/tools/、/faq/ 这些页面在真正上线前好几个月就已经是「合理的猜测」了。现在它会显示到底是哪个路径 404 了，对全部真实的模型/显卡/指南 slug 做编辑距离模糊匹配——/quant-hub/qwen3-8/ 能正确联想到 Qwen3 8B Instruct——一个能真正提交到模型库的搜索框，以及全部六个板块入口带实时计数，而不是四个写死的链接。核实而非假设了 404 页真正要紧的那一件事：模拟了静态托管平台的真实回退行为（找不到文件时用 404.html 返回真正的 HTTP 404 状态，而不是 200 的 SPA 重写），确认状态码确实是 404，也确认中文路径（/zh/xxx/）会渲染出中文版页面——同一个 not-found.tsx 之所以能兼顾两种语言，是因为 usePathname() 在客户端 hydrate 时读的是浏览器地址栏的真实 URL，跟到底是哪个静态文件被拿来兜底无关，这一点是验证过的，不是想当然。noindex 在更早一次上线里就已经加上了，这次只是换了它下面的内容',
  },
  {
    date: '2026-09-13',
    en: '12 legacy models (Zephyr 7B Beta, WizardLM-2 7B, OpenChat 3.6 8B, Aya 23 8B, SOLAR 10.7B, both InternLM2 sizes, DBRX Instruct, Jamba 1.5 Mini, StarCoder2 15B, Command R 35B, Yi 1.5 34B) got the "prefer something else today" banner this index had for five of them, and seven more got it for the first time, each pointing at a real, currently-indexed replacement with a checked reason (not a template phrase): longer context, more quant levels, or — when neither number favours the newer model — that it still gets community quant builds and the old one no longer does. Every banner links straight into the compare tool with both models pre-filled. The Hub now hides all 13 superseded models by default (a toggle brings them back; their pages stay indexed, nothing gets `noindex`) and their meta description now leads with "Legacy model." Running the actual recommendation function instead of just reading it surfaced a real bug this ship also fixes: `homePicks` (the function behind the homepage hero and every `/best/` tier) was recommending superseded models outright — Qwen2-VL-7B held the multimodal recommendation on almost every GPU in the database because nothing had ever told the ranking function a model could be off the table, and DBRX Instruct and StarCoder2 15B were doing the same for "code" on several cards the moment this batch marked them legacy. A recommendation is exactly the one place that must never happen',
    zh: '12 个过时型号（Zephyr 7B Beta、WizardLM-2 7B、OpenChat 3.6 8B、Aya 23 8B、SOLAR 10.7B、InternLM2 的两个尺寸、DBRX Instruct、Jamba 1.5 Mini、StarCoder2 15B、Command R 35B、Yi 1.5 34B）现在都带上了此前只有其中 5 个才有的「今天建议换一个」提示条，另外 7 个是第一次拿到——每一条都指向一个真实存在、目前仍在索引中的替代型号，配的理由是核实过的（不是套模板换个名字）：更长的上下文、更多可选的量化档位，或者——当这两个数字都不占优时——老实说这个替代仍在持续获得社区量化构建，而这个老型号已经没有了。每条提示都直接链接到对比工具并预填好两个型号。模型库现在默认隐藏全部 13 个已过时型号（有开关可以调回来；它们的页面依旧被收录，没有加 noindex），meta description 也会以「过时型号。」开头。真的跑一遍推荐函数、而不是只读代码，这次还揪出了一个真实 bug：`homePicks`（首页主推荐和每个 `/best/` 分级页背后用的都是它）此前会直接推荐已过时的型号——Qwen2-VL-7B 几乎在数据库里每一张显卡上都占着"多模态"推荐位，因为从来没人告诉这个排序函数某个型号可能已经被排除在外；这一批打上过时标签之后，DBRX Instruct 和 StarCoder2 15B 也在好几张卡上对"代码"类别犯了同样的错。推荐，恰恰是这种事最不该发生的地方',
  },
  {
    date: '2026-09-13',
  ...
  },
  {
    ...
    zh: '对导出的每一页做了一次扫描，发现 63 个 <title> 标签超过 60 字符——审计报告里标记的是 33 个，但索引自那以后又扩充了，而且同一个问题还出现在审计没点名的五类模板上：首页、全部六个 /best/ 分级页、/formats/、四个单格式页，以及 /tools/、/faq/、/changelog/。两个由数据驱动的模板问题最严重——29 个模型页标题里都带着「— Quant Variants & VRAM」，16 篇指南标题都拖着一个多余的「 | quantized.uk Cookbook」后缀，22 个字符却没有任何 SERP 价值。修复方式是给模型标题加一个两级回退（完整版，超长则用短版——核对过索引里每一个模型，没有任何一个需要第三级），并给唯一一篇标题本身就超长的指南加了 seoTitle 这个补丁字段；全程没有用运行时省略号截断任何标题。几个静态模板是手动缩短的。顺带修了 /about/：原来是个 20 字符、什么也没说清楚的标题，现在直接点明这页是干什么的。修完后全站统计：0 个标题超过 60 字符，0 个重复',
  },
  {
    date: '2026-09-12',
